...
//    }
        ...
        for (int i=0;i<this.members.length;i++){
            switch (rnd.nextInt(3)){
                case 0:
                    this.members[i]= new Cat(catNames[rnd.nextInt(catNames.length)], rnd.nextInt(2)+1, (double) (rnd.nextInt(2000)+500)/100);
                    break;
                case 1:
                    this.members[i]= new Dog(dogNames[rnd.nextInt(dogNames.length)], rnd.nextInt(3)+1, (double) (rnd.nextInt(4000)+500)/100);
                    break;
                default:
        ...
        for (Competitor member:this.members) {
            if(member.isOnDistance()){
                if (member instanceof Animal){
                    System.out.println(((Animal) member).type+" "+ member.getName()+": прошел.");}
                else {System.out.println(member.getName()+": прошел.");}
            }else {
                    if (member instanceof Animal){
                        System.out.println(((Animal) member).type+" "+ member.getName()+": не прошел.");}
                    else {System.out.println(member.getName()+": не прошел.");}
